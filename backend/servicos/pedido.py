...
class PedidoDatabase():
    ...
    def get_pedidos(self, endereco_filial: str = None, cnpj_fornec: str = None, id_produto: int = None):
        ...
        if cnpj_fornec:
            conditions.append(f"cnpjfornec = '{cnpj_fornec}'")
        if id_produto:
    ...
    def get_pedido(self, endereco_filial: str, cnpj_fornec: str, id_produto: int):
        query = f"SELECT * FROM pedido WHERE enderecofilial = '{endereco_filial}' AND cnpjfornec = '{cnpj_fornec}' AND idproduto = {id_produto}"
    ...
    def cadastra_pedido(self, endereco_filial: str, cnpj_fornec: str, id_produto: int, data_pedido: str, quantidade: str):
        statement = f"INSERT INTO pedido (enderecofilial, cnpjfornec, idproduto, datapedido, quantidade) VALUES ('{endereco_filial}', '{cnpj_fornec}', {id_produto}, '{data_pedido}', '{quantidade}')"
    ...
    def atualiza_pedido(self, endereco_filial: str, cnpj_fornec: str, id_produto: int, data_pedido: str = None, quantidade: str = None):
        updates = []
        if data_pedido:
            updates.append(f"datapedido = '{data_pedido}'")
        if quantidade:
        ...
        statement = f"UPDATE pedido SET {', '.join(updates)} WHERE enderecofilial = '{endereco_filial}' AND cnpjfornec = '{cnpj_fornec}' AND idproduto = {id_produto}"
        return self.db.execute_statement(statement)

    def deleta_pedido(self, endereco_filial: str, cnpj_fornec: str, id_produto: int):
        statement = f"DELETE FROM pedido WHERE enderecofilial = '{endereco_filial}' AND cnpjfornec = '{cnpj_fornec}' AND idproduto = {id_produto}"
        return self.db.execute_statement(statement)
